...
THIS TUTORIAL COMPLETED BY: Emily Perez-Medina
...
fivethirtyeight:: #see what data frames are in the package
```
...
```{r}
sweetnesschocolate <- ggplot(candy_rankings)+
  aes(x=chocolate,y=sugarpercent)+
  geom_boxplot()
sweetnesschocolate

pricefruity <- ggplot(candy_rankings)+
...
2.  Are sweeter candies more expensive? Do the candies with higher sugarpercent have higher winpercent? Plot this relationship.
    ...
    ```{r}
    sweetnessprice <- ggplot(candy_rankings) + 
...
```{r}
sweetnesswin <- ggplot(candy_rankings) + 
  aes(x =sugarpercent , y = winpercent) + 
  geom_point()
